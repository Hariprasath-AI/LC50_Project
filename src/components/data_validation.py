# In this python file we're going to check whether the incoming data is in the format of csv or in any other form.
...
class DataValidation:
    def validate(data_loc):
        try:
            column_names = ['CIC0','SM1_Dz(Z)','GATS1i','NdsCH','NdssC','MLOGP', 'LC50']
            data = pd.read_csv(data_loc, , header = None, delimiter = ';', names = column_names)
            logging.info("There is no problem with the data. So, we can continue further.")
            return data

        except Exception as e:
            logging.info("Error occured while importing the data. Please check format of the data i.e., csv")
